...
export const protect = catchAsync(async (req, res, next) => {
	...
	//{ id: '6430212a09c149903d033b0b', iat: 1680875818, exp: 1688651818 }
...
export const forgotPassword = catchAsync(async (req, res, next) => {
	...
	try {
		...
		// using try/catch instead of AppError alone, because we are modifing user model
...
export const resetPassword = catchAsync(async (req, res, next) => {
	...
	// encrypt the original password to compare to the passwords saved in the database
	const hashedToken = crypto
		.createHash('sha256')
		.update(req.params.token)
		.digest('hex');

	// get user based on the passwordResetToken
	// if the passwordResetExpires is greather than the time at which the token was issued
	...
	// the user exist?, set the new password
	user.password = req.body.password;
	user.passwordConfirm = req.body.passwordConfirm;
	// when setting to undefined, thet don't show in the database
	user.passwordResetToken = undefined;
	user.passwordResetExpires = undefined;
